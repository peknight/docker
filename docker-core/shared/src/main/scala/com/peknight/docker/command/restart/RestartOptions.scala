package com.peknight.docker.command.restart
...
case class RestartOptions() extends DockerOptions:
  def options: List[String] = this.toOptions
end RestartOptions
object RestartOptions:
  def default: RestartOptions = RestartOptions()
  given codecRestartOptions[F[_]: Monad, S: {ObjectType, NullType, StringType, Show}]: Codec[F, S, Cursor[S], RestartOptions] =
    Codec.derived[F, S, RestartOptions]
end RestartOptions
